...
public:
    static storage::FetchMeta build(
        ...
        const std::string& etag, // ETag header value
        const std::string& last_modified, // Last-Modified header value
        int crawl_depth, // depth of the crawl, e.g., 0 for seed URLs, 1 for first-level links, etc.
        const std::string& referrer, // URL of the page that linked to this document

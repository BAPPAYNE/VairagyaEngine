...
QualitySignals QualitySignalsBuilder::build(const std::string& clean_text, time_t last_changed) {
...
float QualitySignalsBuilder::calculateReadability(const std::string& text) {
    if (text.empty()) return 0.0f;
    // Simple Flesch-Kincaid style approximation
    float words = 1.0f;
    for (char c : text) if (c == ' ') words++;
    float chars = (float)text.length();
    return std::clamp(1.0f - (words / chars), 0.0f, 1.0f);
}

float QualitySignalsBuilder::calculateSpamScore(const std::string& text) {
